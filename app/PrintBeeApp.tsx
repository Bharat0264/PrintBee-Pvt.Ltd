              ...
              <div className="dashboard-block">
                ...
                <section className="ledger-section" id="admin-ledger">
                  ...
                  {!ledger ? <form className="ledger-lock" onSubmit={(event) => { event.preventDefault(); unlockLedger(); }}><label>Ledger password<input type="password" autoComplete="current-password" value={ledgerPassword} onChange={(event) => setLedgerPassword(event.target.value)} placeholder="Enter password" /></label><button disabled={!ledgerPassword}>Unlock ledger</button>{ledgerMessage && <p>{ledgerMessage}</p>}</form> : <>
                    ...
                    <p className="ledger-note">Printing revenue uses the amount collected for printing. Payment handling charges are recorded as payment-gateway operating cost. The operator receives 65% of printing profit and profit from other printing services such as binding. The owner receives the remaining printing profit, platform fees, packing profit, surcharge, project platform revenue, and all other profit; late-night delivery is split 60% to the delivery partner and 40% to the owner.</p>
